...
import { registerWithCredentials } from "@/app/actions/auth"; // adjust import path
...
export default function RegisterPage() {
  ...
  return (
    <div className="relative min-h-screen flex flex-col items-center justify-center bg-brand-black text-brand-white font-sans antialiased overflow-x-hidden selection:bg-mint-500/20 px-4">
      {/* Grid overlay – identical to all WealthWatch pages */}
      ...
      {/* Minimal top bar with brand link */}
      ...
      <div className="w-full max-w-md border border-white/10 p-8 md:p-10 bg-gray-950/50 backdrop-blur-sm">
        <h1 className="text-2xl font-light tracking-[-0.02em] mb-2">Create your account</h1>
        <p className="text-sm text-gray-400 mb-8">Start tracking your wealth in under 60 seconds.</p>
        ...
        <form action={handleFormAction} className="flex flex-col gap-4">
          <div className="flex flex-col gap-1.5">
            <label htmlFor="name" className="text-xs font-mono text-gray-500 tracking-[0.08em] uppercase">
              Full name
            </label>
            <input
              id="name"
        ...
        <p className="mt-6 text-center text-xs text-gray-500 font-mono">
          ...
          <Link href="/login" className="text-sky-400 underline underline-offset-2 hover:text-sky-300 transition-colors">
            Sign in
          </Link>
        </p>
      </div>
